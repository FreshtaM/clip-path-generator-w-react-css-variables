function _defineProperty(obj, key, value) {if (key in obj) {Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true });} else {obj[key] = value;}return obj;}const { Component, Fragment } = React;
const { render } = ReactDOM;

const getClipPath = (nodes, size, mode) => {
  let path = '';
  let centerX;
  let centerY;
  let dragX;
  let dragY;
  const getRatio = d => Math.floor(d / size * 100);
  if (!nodes.length) return null;
  switch (mode) {
    case MODES.POLYGON:
      for (let n = 0; n < nodes.length; n++) {
        const { x, y } = nodes[n];
        path += `${getRatio(x)}% ${getRatio(y)}%${
        n === nodes.length - 1 ? '' : `, `
        }`;
      }
      break;
    case MODES.INSET:
      for (let n = 0; n < nodes.length; n++) {
        const { x, y } = nodes[n];
        switch (n) {
          case 0:
            path += `${getRatio(y)}% `;
            break;
          case 1:
            path += `${100 - getRatio(x)}% `;
            break;
          case 2:
            path += `${100 - getRatio(y)}% `;
            break;
          case 3:
            path += `${getRatio(x)}%`;
            break;}

      }
      break;
    case MODES.ELLIPSE:
      ;({ x: centerX, y: centerY } = nodes[0]);
      dragX = nodes[1].x;
      dragY = nodes[2].y;
      const xDistance = Math.abs(getRatio(dragX - centerX));
      const yDistance = Math.abs(getRatio(dragY - centerY));
      path = `${xDistance}% ${yDistance}% at ${getRatio(centerX)}% ${getRatio(
      centerY)
      }%`;
      break;
    case MODES.CIRCLE:
      ;({ x: centerX, y: centerY } = nodes[0]);
      dragX = nodes[1].x;
      dragY = nodes[1].y;
      const distX = dragX - centerX;
      const distY = dragY - centerY;
      const distance = getRatio(Math.sqrt(distX * distX + distY * distY));
      path = `${distance}% at ${getRatio(centerX)}% ${getRatio(centerY)}%`;
      break;
    default:
      path = 'No mode selected';}

  return `${mode.toLowerCase()}(${path})`;
};

class ClipPathNode extends Component {constructor(...args) {super(...args);_defineProperty(this, "state",
    {
      moving: false,
      x: this.props.x,
      y: this.props.y });_defineProperty(this, "componentDidMount",

    () => {
      const { el, startMove, removeNode } = this;
      el.addEventListener('mousedown', startMove);
      el.addEventListener('touchstart', startMove);
      el.addEventListener('click', removeNode);
    });_defineProperty(this, "removeNode",
    () => {
      const { id, removing, onRemove } = this.props;
      if (removing && onRemove) onRemove(id);
    });_defineProperty(this, "getPageXY",
    e => {
      let { pageX: x, pageY: y, touches } = e;
      if (touches && touches.length === 1) {
        x = touches[0].pageX;
        y = touches[0].pageY;
      }
      y -= window.pageYOffset;
      return {
        x,
        y };

    });_defineProperty(this, "startMove",
    e => {
      const { onMove, removing, restrictX, restrictY } = this.props;
      if (removing) return;
      const { x, y } = this.getPageXY(e);
      const move = e => {
        const { x: oldX, y: oldY } = this.state;
        const { x: pageX, y: pageY } = this.getPageXY(e);
        const {
          height,
          top,
          left,
          width } =
        this.el.parentNode.getBoundingClientRect();
        const x = Math.min(Math.max(0, pageX - left), width);
        const y = Math.min(Math.max(0, pageY - top), height);
        this.setState(
        {
          x: restrictX ? oldX : x,
          y: restrictY ? oldY : y },

        () => onMove(this));

      };
      const endMove = () => {
        this.setState(
        {
          moving: false },

        () => {
          onMove(this);
          document.body.removeEventListener('mousemove', move);
          document.body.removeEventListener('mouseup', endMove);
          if (e.touches && e.touches.length === 1) {
            document.body.removeEventListener('touchmove', move);
            document.body.removeEventListener('touchend', endMove);
          }
        });

      };
      const initMove = () => {
        document.body.addEventListener('mousemove', move);
        document.body.addEventListener('mouseup', endMove);
        if (e.touches && e.touches.length === 1) {
          document.body.addEventListener('touchmove', move);
          document.body.addEventListener('touchend', endMove);
        }
      };
      this.setState(
      {
        moving: true },

      initMove);

    });_defineProperty(this, "componentWillReceiveProps",
    nextProps => {
      if (nextProps.x !== this.props.x || nextProps.y !== this.props.y) {
        this.setState({ x: nextProps.x, y: nextProps.y });
      }
    });_defineProperty(this, "render",
    () => {
      const { moving, x, y } = this.state;
      const { removing } = this.props;
      return /*#__PURE__*/(
        React.createElement("div", {
          className: `clip-path-node ${moving ? 'clip-path-node--moving' : ''} ${
          removing ? 'clip-path-node--removing' : ''
          }`,
          ref: n => this.el = n,
          style: {
            '--x': x,
            '--y': y } },

        removing && /*#__PURE__*/
        React.createElement("svg", { className: "clip-path-node__remove", viewBox: "0 0 24 24" }, /*#__PURE__*/
        React.createElement("path", { d: "M19,6.41L17.59,5L12,10.59L6.41,5L5,6.41L10.59,12L5,17.59L6.41,19L12,13.41L17.59,19L19,17.59L13.41,12L19,6.41Z" }))));




    });}}

const SIZE = 300;
const PRESETS = {
  CIRCLE: [
  {
    x: 195,
    y: 105 },

  {
    x: 140,
    y: 50 }],


  ELLIPSE: [
  {
    x: 195,
    y: 150 },

  {
    x: 285,
    y: 150 },

  {
    x: 195,
    y: 30 }],


  INSET: [
  {
    x: 195,
    y: 30 },

  {
    x: 270,
    y: 165 },

  {
    x: 195,
    y: 300 },

  {
    x: 120,
    y: 165 }],


  POLYGON: [
  {
    x: 255,
    y: 20 },

  {
    x: 210,
    y: 45 },

  {
    x: 90,
    y: 45 },

  {
    x: 45,
    y: 15 },

  {
    x: 0,
    y: 75 },

  {
    x: 30,
    y: 135 },

  {
    x: 30,
    y: 225 },

  {
    x: 75,
    y: 270 },

  {
    x: 120,
    y: 285 },

  {
    x: 180,
    y: 285 },

  {
    x: 225,
    y: 275 },

  {
    x: 270,
    y: 225 },

  {
    x: 270,
    y: 135 },

  {
    x: 300,
    y: 75 }] };



const MODES = {
  CIRCLE: 'CIRCLE',
  ELLIPSE: 'ELLIPSE',
  INSET: 'INSET',
  POLYGON: 'POLYGON' };

class ClipPathGenerator extends Component {constructor(...args) {super(...args);_defineProperty(this, "state",
    {
      removing: false,
      mode: MODES.POLYGON,
      size: SIZE,
      nodes: PRESETS[MODES.POLYGON] });_defineProperty(this, "onUpdate",

    n => {
      const { id: updatedId } = n.props;
      const { x: updatedX, y: updatedY } = n.state;
      const { mode, nodes: currentNodes } = this.state;
      const getHandlePoint = (node, axis) => {
        const movement = n.state[axis] - currentNodes[0][axis];
        let newPoint = node[axis] + movement;
        if (newPoint > SIZE || newPoint < 0) {
          newPoint = currentNodes[0][axis] + currentNodes[0][axis] - newPoint;
        }
        return Math.min(SIZE, Math.max(0, newPoint));
      };
      // Create a new set of nodes by mapping through the current node set
      const nodes = currentNodes.map((o, idx) => {
        // If it's the node that's being moved, just update it with new values
        if (idx === updatedId) {
          o = Object.assign({}, o, {
            x: updatedX,
            y: updatedY });

        }
        // Else if in INSET mode update the corresponding axis so the nodes are center aligned
        else if (mode === MODES.INSET) {
            // Need to update the opposite axis to still be centrally aligned in the clip

            const getCenterPoint = (start, end, axis) =>
            currentNodes[start][axis] +
            (currentNodes[end][axis] - currentNodes[start][axis]) / 2;
            // UPDATING X
            if (updatedId % 2 && !(idx % 2)) {
              o = Object.assign({}, o, {
                x:
                updatedId === 1 ?
                getCenterPoint(3, 1, 'x') :
                getCenterPoint(1, 3, 'x') });

            } else if (!(updatedId % 2) && idx % 2) {
              o = Object.assign({}, o, {
                y:
                updatedId === 2 ?
                getCenterPoint(0, 2, 'y') :
                getCenterPoint(2, 0, 'y') });

            }
          } else if (mode === MODES.ELLIPSE && updatedId === 0 && idx !== 0) {
            o = Object.assign({}, o, {
              y: idx === 1 ? updatedY : getHandlePoint(o, 'y'),
              x: idx === 1 ? getHandlePoint(o, 'x') : updatedX });

          } else if (mode === MODES.CIRCLE && updatedId === 0 && idx !== 0) {
            o = Object.assign({}, o, {
              x: getHandlePoint(o, 'x'),
              y: getHandlePoint(o, 'y') });

          }
        return o;
      });
      this.setState({
        copied: false,
        nodes });

    });_defineProperty(this, "addNode",
    () => {
      this.setState({
        copied: false,
        nodes: [
        ...this.state.nodes,
        {
          x: SIZE / 2,
          y: SIZE / 2 }] });



    });_defineProperty(this, "removeNode",
    id => {
      const nodes = this.state.nodes.filter((n, i) => i !== id);
      this.setState({
        nodes });

    });_defineProperty(this, "removeNodes",
    () => {
      this.setState({
        copied: false,
        removing: !this.state.removing });

    });_defineProperty(this, "switchMode",
    e => {
      this.setState({
        copied: false,
        mode: MODES[e.target.value.toUpperCase()],
        nodes: PRESETS[e.target.value.toUpperCase()] });

    });_defineProperty(this, "copy",
    () => {
      if (this.path) {
        this.path.select();
        document.execCommand('Copy');
        this.setState({
          copied: true });

      }
    });_defineProperty(this, "wipeNodes",
    () => {
      this.setState({
        nodes: [] });

    });_defineProperty(this, "render",
    () => {
      const {
        addNode,
        copy,
        switchMode,
        onUpdate,
        removeNode,
        removeNodes,
        wipeNodes } =
      this;
      const { copied, mode, nodes, removing, size } = this.state;
      const clipPath = getClipPath(nodes, size, mode);
      return /*#__PURE__*/(
        React.createElement(Fragment, null, /*#__PURE__*/
        React.createElement("div", { className: "clip-path-generator__container" }, /*#__PURE__*/
        React.createElement("div", { className: "clip-path-generator", ref: c => this.el = c }, /*#__PURE__*/
        React.createElement("div", {
          className: "clip-path-generator__clipped",
          style: {
            '--path': clipPath } }),


        nodes.map((n, idx) => /*#__PURE__*/
        React.createElement(ClipPathNode, {
          id: idx,
          key: `clip-node--${idx}`,
          x: n.x,
          y: n.y,
          removing: removing,
          onMove: onUpdate,
          onRemove: removeNode,
          restrictX:
          mode === MODES.INSET && !(idx % 2) ||
          mode === MODES.ELLIPSE && idx === 2 ?
          true :
          false,

          restrictY:
          mode === MODES.INSET && idx % 2 ||
          mode === MODES.ELLIPSE && idx === 1 ?
          true :
          false })))), /*#__PURE__*/





        React.createElement("div", { className: "clip-path" }, /*#__PURE__*/
        React.createElement("div", { className: "clip-path__value" }, clipPath ? `clip-path: ${clipPath};` : 'No clip path set'), /*#__PURE__*/
        React.createElement("input", {
          className: "clip-path__input",
          readOnly: true,
          ref: p => this.path = p,
          value: `clip-path: ${clipPath};` }), /*#__PURE__*/

        React.createElement("button", {
          disabled: removing || !clipPath,
          className: "clip-path__copy button",
          onClick: copy },
        copied ? 'Copied 👍' : 'Copy')), /*#__PURE__*/


        React.createElement("div", { className: "clip-path-options" },
        Object.keys(MODES).map((m, idx) => /*#__PURE__*/
        React.createElement("div", { key: `clip-path-option--${idx}`, className: "clip-path-option" }, /*#__PURE__*/
        React.createElement("input", {
          disabled: removing,
          className: "clip-path-option__input",
          type: "radio",
          name: "shape",
          value: m,
          id: `clip-path-option--${m}`,
          onChange: switchMode,
          checked: m === mode }), /*#__PURE__*/

        React.createElement("label", {
          className: "clip-path-option__label",
          htmlFor: `clip-path-option--${m}` }, /*#__PURE__*/
        React.createElement("div", { className: "clip-path-option__check" }),
        `${m.charAt(0)}${m.toLowerCase().substr(1)}`)))),




        mode === MODES.POLYGON && /*#__PURE__*/
        React.createElement("div", { className: "polygon-actions" }, /*#__PURE__*/
        React.createElement("button", {
          className: "polygon-action polygon-action--add button",
          disabled: removing,
          onClick: addNode }, "Add Node"), /*#__PURE__*/


        React.createElement("button", {
          className: `polygon-action ${
          removing ? 'polygon-action--removing' : 'polygon-action--remove'
          } button`,
          onClick: removeNodes },
        removing ? 'Done' : 'Remove Node'), /*#__PURE__*/

        React.createElement("button", {
          disabled: removing,
          className: "polygon-action polygon-action--wipe button",
          onClick: wipeNodes },
        'Wipe Nodes'))));





    });}}

render( /*#__PURE__*/React.createElement(ClipPathGenerator, null), document.getElementById('app'));